...
class solydxkRemoveWindow:

    def __init__(self, solydxkFile):
        self.swmFile = solydxkFile

        if os.path.exists(self.swmFile):
            directory = home + "/.solydxk/softwaremanager/tmp/solydxkFile"
            os.system("mkdir -p " + directory)
            os.system("rm -rf " + directory + "/*")
            os.system("cp " + self.swmFile + " " + directory + "/file.swm")
            ...
            #Set the Glade file
            self.gladefile = "/usr/share/solydxk/softwaremanager/remove.glade"
...
if __name__ == "__main__":
    mainwin = solydxkRemoveWindow(sys.argv[1])
