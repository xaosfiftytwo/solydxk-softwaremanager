...
def show_item(selection, model, wTree, username):
    ...
    if (iter is not None):
        ...
        if (selected_item.screenshot is not None):
...
def open_search(widget, username):
    os.system("/usr/lib/solydxk/softwaremanager/softwaremanager-args.py " + username + " &")

def open_featured(widget):
    gladefile = "/usr/share/solydxk/softwaremanager/frontend.glade"
    ...
    wTree.get_widget("featured_window").set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
    ...
    import string
    applications = open("/usr/share/solydxk/softwaremanager/featured_applications/list.txt", "r")
        ...
        if len(application_details) == 3:
            ...
            try:
                ...
                if ((not pkg.is_installed) and (pkg.candidate.summary != "")):
                    ...
                    model.set_value(iter, 3, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/featured_applications/" + application_icon))
...
def install_featured(widget, wTree, treeview_featured, window):
    ...
    command = "gksu /usr/lib/solydxk/softwaremanager/synaptic-install " + window_id
...
def show_screenshot(widget, model):
    #Set the Glade file
    if model.selected_application is not None:
        gladefile = "/usr/share/solydxk/softwaremanager/frontend.glade"
        ...
        wTree.get_widget("screenshot_window").set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
...
def fetch_apt_details(model):
    global cache
    if os.path.exists("/usr/share/solydxk/softwaremanager/data/details/packages.list"):
        packagesFile = open("/usr/share/solydxk/softwaremanager/data/details/packages.list", "r")
...
def show_more_info(widget, model):
    if model.selected_application is not None:
        if not os.path.exists((model.selected_application.swm_file)):
            os.system("zenity --error --text=\"" + _("The solydxk file for this application was not successfully downloaded. Click on refresh to fix the problem.") + "\"")
        else:
            directory = home + "/.solydxk/softwaremanager/tmp/solydxkFile"
            os.system("mkdir -p " + directory)
            os.system("rm -rf " + directory + "/*")
            os.system("cp " + model.selected_application.swm_file + " " + directory + "/file.swm")
            os.system("tar zxf " + directory + "/file.swm -C " + directory)
            ...
            wTree.get_widget("more_info_window").set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
...
def visit_web(widget, model, username):
    if model.selected_application is not None:
        functions.showWebsite(username, model.selected_application.link)
    ...
    if model.selected_application is not None:
        if model.selected_application.is_special:
            if not os.path.exists((model.selected_application.swm_file)):
                ...
                os.system("softwaremanager " + model.selected_application.swm_file)
            ...
            model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/status-icons/add.png"))
    ...
    if model.selected_application is not None:
        if model.selected_application.is_special:
            if not os.path.exists((model.selected_application.swm_file)):
                ...
                os.system("/usr/lib/solydxk/softwaremanager/remove.py " + model.selected_application.swm_file)
                show_item(wTree.get_widget("tree_applications").get_selection(), model, wTree, username)
                global cache
                cache = apt.Cache()
                show_applications(wTree, model, False)
            ...
            wTree.get_widget("toolbutton_apply").set_sensitive(True)
            model_applications, iter = wTree.get_widget("tree_applications").get_selection().get_selected()
            model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/status-icons/remove.png"))
...
def cancel_change(widget, model, wTree, username):
    if model.selected_application is not None:
        ...
        if model.selected_application.status == "add":
            ...
            model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/status-icons/available.png"))
            ...
            model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/status-icons/installed.png"))
        ...
        for item in portal.items:
            if (item.category.key in category_keys):
                if item.status in matching_statuses and (model.keyword == None
                    ...
                    if item.is_special:
                        model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/status-icons/special.png"))
...
def update_statusbar(wTree, model):
    ...
    context_id = statusbar.get_context_id("softwaremanager")
...
def open_about(widget):
    dlg = gtk.AboutDialog()
    dlg.set_version(functions.getPackageVersion("softwaremanager"))
    dlg.set_name("softwaremanager")
...
class DownloadScreenshot(threading.Thread):
    ...
    def run(self):
        try:
            #import urllib
            urllib.urlretrieve (self.selected_item.screenshot_url, "/usr/share/solydxk/softwaremanager/data/screenshots/" + self.selected_item.key)
...
class RefreshThread(threading.Thread):
    ...
    def __init__(self, wTree, refresh, model, username):
        ...
        self.directory = "/usr/share/solydxk/softwaremanager/data"
        self.model = model
    ...
    def run(self):
        try:
            ...
            try:
                ...
                if allPortalsHere:
                    print details
                    os.system("zenity --error --text=\"" + _("The data used by softwaremanager is corrupted or out of date. Click on refresh to fix the problem :") + " " + str(details) + "\"")
                else:
                    gtk.gdk.threads_enter()
                    dialog = gtk.MessageDialog(self.wTree.get_widget("main_window"), gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_NONE, _("Please refresh softwaremanager by clicking on the Refresh button"))
                    dialog.set_title("softwaremanager")
                    dialog.set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
    ...
    def initialize(self):
        ...
        os.system("mkdir -p " + self.directory + "/solydxkfiles")
        ...
        #    os.system("cp /etc/solydxk/version/softwaremanager/portals.list " + self.directory + "/etc/portals.list")

    def register_portals(self, model):
        portalsFile = open("/usr/share/solydxk/softwaremanager/portals.list")
        ...
        for element in root:
            if element.tag == "category":
                ...
                if self.refresh:
                    ...
                    os.chdir("/usr/lib/solydxk/softwaremanager")
                ...
                item = Classes.Item(portal, element.attrib["id"], element.attrib["link"], element.attrib["solydxk_file"], element.attrib["category"], element.attrib["name"], element.attrib["description"], "", element.attrib["added"], element.attrib["views"], element.attrib["license"], element.attrib["size"], element.attrib["website"], element.attrib["repository"], element.attrib["average_rating"])
                ...
                if self.refresh:
                    os.chdir(self.directory + "/solydxkfiles")
                    os.system("wget -nc -O" + item.key + ".swm -T10 \"" + item.swm_file + "\"")
                    os.chdir("/usr/lib/solydxk/softwaremanager")
                item.swm_file = self.directory + "/solydxkfiles/" + item.key + ".swm"
    ...
    def load_model_in_GUI(self, wTree, model):
        ...
        model_categories.set_value(iter, 2, gtk.gdk.pixbuf_new_from_file_at_size("/usr/share/solydxk/softwaremanager/icon.svg", 16, 16))
        for portal in model.portals:
            for category in portal.categories:
