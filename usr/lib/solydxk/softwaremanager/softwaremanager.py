...
# Don't let softwaremanager run as root
#~ if os.getuid() == 0:
...
sys.path.append('/usr/lib/solydxk/common')
cfgLocal = Config(home + "/.solydxk/softwaremanager.conf")
...
class DownloadReviews(threading.Thread):
    ...
    def run(self):
        try:
            reviews_dir = home + "/.solydxk/softwaremanager"
            os.system("mkdir -p " + reviews_dir)
            reviews_path = reviews_dir + "/reviews.list"
            reviews_path_tmp = reviews_path + ".tmp"
            urllib.urlretrieve("http://community.linuxmint.com/data/reviews.list", reviews_path_tmp)
            numlines = 0
            numlines_new = 0
...
class Application():
    ...
    def __init__(self):
        ...
        # Build the GUI
        gladefile = "/usr/share/solydxk/softwaremanager/softwaremanager.glade"
        ...
        if os.path.exists("/usr/bin/software-sources") or os.path.exists("/usr/bin/software-properties-gtk") or os.path.exists("/usr/bin/software-properties-kde"):
            ...
            sourcesMenuItem.set_image(gtk.image_new_from_file("/usr/share/solydxk/softwaremanager/icons/software-properties.png"))
        ...
        template = open("/usr/share/solydxk/softwaremanager/data/templates/CategoriesView.html").read()
        ...
        template = open("/usr/share/solydxk/softwaremanager/data/templates/SubCategoriesView.html").read()
        subs = {'header': _("Categories")}
        ...
        self.generic_installed_icon_path = "/usr/share/solydxk/softwaremanager/data/installed.png"
        self.generic_available_icon_path = "/usr/share/solydxk/softwaremanager/data/available.png"
    ...
    def set_filter(self, checkmenuitem, configName):
        cfgLocal.setValue('filter', configName, checkmenuitem.get_active())
        self.prefs = self.read_configuration()
        if self.model_filter is not None:
            self.model_filter.refilter()

    def set_search_filter(self, checkmenuitem, configName):
        cfgLocal.setValue('search', configName, checkmenuitem.get_active())
        self.prefs = self.read_configuration()
        if (self.searchentry.get_text() != ""):
            self.show_search_results(self.searchentry.get_text())

    def set_external_browser(self, checkmenuitem):
        cfgLocal.setValue('softwaremanager', 'external_browser', checkmenuitem.get_active())
        self.prefs = self.read_configuration()
    ...
    def read_configuration(self):
        ...
        try:
            section = 'account'
            prefs["username"] = cfgLocal.getValue(section, 'username')
            prefs["password"] = cfgLocal.getValue(section, 'password')
        except:
            prefs["username"] = ""
            prefs["password"] = ""
        ...
        section = 'filter'
        try:
            prefs["available_packages_visible"] = (cfgLocal.getValue(section, 'available_packages_visible') == "True")
        except:
            prefs["available_packages_visible"] = True
        try:
            prefs["installed_packages_visible"] = (cfgLocal.getValue(section, 'installed_packages_visible') == "True")
        except:
        ...
        section = 'search'
        try:
            prefs["search_in_summary"] = (cfgLocal.getValue(section, 'search_in_summary') == "True")
        except:
            prefs["search_in_summary"] = True
        try:
            prefs["search_in_description"] = (cfgLocal.getValue(section, 'search_in_description') == "True")
        except:
            prefs["search_in_description"] = False
        try:
            prefs["search_while_typing"] = (cfgLocal.getValue(section, 'search_while_typing') == "True")
        except:
            prefs["search_while_typing"] = True

        #External browser
        try:
            prefs["external_browser"] = (cfgLocal.getValue('softwaremanager', 'external_browser') == "True")
        except:
    ...
    def open_account_info(self, widget):
        ...
        wTree.get_widget("window_account").set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
    ...
    def update_account_info(self, entry, prop, configName):
        ...
        cfgLocal.setValue('account', configName, text)
        self.prefs = self.read_configuration()

    def open_about(self, widget):
        dlg = gtk.AboutDialog()
        dlg.set_title(_("About"))
        dlg.set_program_name("softwaremanager")
        ...
        try:
            ...
            print detail
        try:
            version = functions.getPackageVersion("softwaremanager")
            dlg.set_version(version)
        except Exception, detail:
            print detail

        dlg.set_authors(["Clement Lefebvre <root@linuxmint.com>"])
        dlg.set_icon_from_file("/usr/share/solydxk/softwaremanager/icon.svg")
        dlg.set_logo(gtk.gdk.pixbuf_new_from_file("/usr/share/solydxk/softwaremanager/icon.svg"))
    ...
    def _on_package_load_finished(self, view, frame, package):
        ...
        if len(reviews) > 10:
            for review in reviews[0:10]:
                rating = "/usr/share/solydxk/softwaremanager/data/small_" + str(review.rating) + ".png"
    ...
    def on_screenshot_clicked(self, url):
        package = self.current_package
        if package is not None:
            template = open("/usr/share/solydxk/softwaremanager/data/templates/ScreenshotView.html").read()
            subs = {}
            subs['url'] = url
            print "loading: '%s'" % url
            html = string.Template(template).safe_substitute(subs)
    ...
    def on_reviews_clicked(self):
        package = self.current_package
        if package is not None:
            template = open("/usr/share/solydxk/softwaremanager/data/templates/ReviewsView.html").read()
            subs = {}
    ...
    def add_categories(self):
        ...
        featured = Category(_("Featured"), "/usr/share/solydxk/softwaremanager/data/templates/featured.svg", None, self.root_category, self.categories)
        featured.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/featured.list")
        ...
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/internet-web.list")
        subcat = Category(_("Email"), "applications-mail", ("mail"), internet, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/internet-email.list")
        subcat = Category(_("Chat"), "xchat", None, internet, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/internet-chat.list")
        subcat = Category(_("File sharing"), "transmission", None, internet, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/internet-filesharing.list")

        cat = Category(_("Sound and video"), "applications-multimedia", ("multimedia", "video"), self.root_category, self.categories)
        cat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/sound-video.list")

        graphics = Category(_("Graphics"), "applications-graphics", ("graphics"), self.root_category, self.categories)
        graphics.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics.list")
        subcat = Category(_("3D"), "blender", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-3d.list")
        subcat = Category(_("Drawing"), "gimp", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-drawing.list")
        subcat = Category(_("Photography"), "shotwell", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-photography.list")
        subcat = Category(_("Publishing"), "scribus", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-publishing.list")
        subcat = Category(_("Scanning"), "flegita", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-scanning.list")
        subcat = Category(_("Viewers"), "gthumb", None, graphics, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/graphics-viewers.list")
        ...
        games.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games.list")
        subcat = Category(_("Board games"), "gnome-glchess", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-board.list")
        subcat = Category(_("First-person shooters"), "UrbanTerror", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-fps.list")
        subcat = Category(_("Real-time strategy"), "applications-games", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-rts.list")
        subcat = Category(_("Turn-based strategy"), "wormux", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-tbs.list")
        subcat = Category(_("Emulators"), "wine", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-emulators.list")
        subcat = Category(_("Simulation and racing"), "torcs", None, games, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/games-simulations.list")
        ...
        cat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/system-tools.list")

        subcat = Category(_("Fonts"), "applications-fonts", ("fonts"), self.root_category, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/fonts.list")

        subcat = Category(_("Science and Education"), "applications-science", ("science", "math", "education"), self.root_category, self.categories)
        subcat.matchingPackages = self.file_to_array("/usr/lib/solydxk/softwaremanager/categories/education.list")
    ...
    def add_reviews(self):
        reviews_path = home + "/.solydxk/softwaremanager/reviews.list"
        if not os.path.exists(reviews_path):
            # No reviews found, use the ones from the packages itself
            os.system("cp /usr/lib/solydxk/softwaremanager/reviews.list %s" % reviews_path)
    ...
    def display_packages_list(self, packages_list, searchTree):
        ...
        for package in packages_list:
            ...
            if package.num_reviews > 0:
                image = "/usr/share/solydxk/softwaremanager/data/" + str(package.avg_rating) + ".png"
                im=Image.open(image)
    ...
    def find_app_icon_alternative(self, package):
        icon_path = None
        if package.pkg.is_installed:
            icon_path = "/usr/share/solydxk/softwaremanager/installed/%s" % package.name
            ...
            if theme.has_icon(package.name):
                ...
                # Try softwaremanager-icons then
                icon_path = "/usr/share/solydxk/softwaremanager/icons/%s" % package.name
                if os.path.exists(icon_path + ".png"):
                    icon_path = icon_path + ".png"
                elif os.path.exists(icon_path + ".xpm"):
                    icon_path = icon_path + ".xpm"
    ...
    def find_app_icon(self, package):
        ...
        if icon_path is not None:
            if package.pkg.is_installed:
                ...
                bg_w, bg_h = im.size
                im2 = Image.open("/usr/share/solydxk/softwaremanager/data/emblem-installed.png")
            ...
            # Try softwaremanager-icons then
            if package.pkg.is_installed:
                icon_path = "/usr/share/solydxk/softwaremanager/icons/installed/%s" % package.name
            else:
                icon_path = "/usr/share/solydxk/softwaremanager/icons/%s" % package.name
    ...
    def show_package(self, package, tree):
        ...
        if package.pkg.is_installed:
            ...
            subs['iconstatus'] = "/usr/share/solydxk/softwaremanager/data/installed.png"
        else:
            ...
            subs['iconstatus'] = "/usr/share/solydxk/softwaremanager/data/available.png"
        ...
        if package.num_reviews > 0:
            ...
            subs['rating'] = "/usr/share/solydxk/softwaremanager/data/no-reviews.png"
            subs['reviews'] = ""

        template = open("/usr/share/solydxk/softwaremanager/data/templates/PackageView.html")
...
if __name__ == "__main__":
    os.system("mkdir -p " + home + "/.solydxk/softwaremanager/screenshots/")
    #splash_process = Popen("/usr/lib/solydxk/softwaremanager/splash.py")
    model = Classes.Model()
    Application()
